...
pcl::clustering<PointT>::clustering(float voxel_resolution, float seed_resolution) :
...
template <typename PointT>
pcl::clustering<PointT>::~clustering()
{
}
/////////////////////////////////////////////////
template <typename PointT> void
pcl::clustering<PointT>::setInputCloud(const typename pcl::PointCloud<PointT>::ConstPtr& cloud)
{
	if (cloud->size() == 0)
	{
	...
	input_ = cloud;//PCLBase
	adjacency_octree_->setInputCloud(cloud);
}
template <typename PointT> void
pcl::clustering<PointT>::setNormalCloud(typename NormalCloudT::ConstPtr normal_cloud)
{
...
/////////////////////////////////////////////////
template <typename PointT> void
pcl::clustering<PointT>::extract(std::map<uint32_t, typename Supervoxel<PointT>::Ptr > &supervoxel_clusters)
{
	//timer_.reset ();
	//double t_start = timer_.getTime ();
	//std::cout << "Init compute  \n";
	bool segmentation_is_possible = initCompute();//PCLBase
	if (!segmentation_is_possible)
	{
		deinitCompute();//PCLBase
		return;
	}

	//std::cout << "Preparing for segmentation \n";
	segmentation_is_possible = prepareForSegmentation();//获取体素中心点云以及对应法向量
	if (!segmentation_is_possible)
	{
		deinitCompute();
		return;
	...
	selectInitialSupervoxelSeeds(seed_indices);//经Rseach过滤后的种子
	//std::cout << "Creating helpers "<<std::endl;
	createSupervoxelHelpers(seed_indices);//创建超体素集（标签，父节点），相当于每个种子体素先作为超体素
	...
	expandSupervoxels(max_depth);//以深度扩展，扩展迭代一次就更新一次中心
	//double t_iterate = timer_.getTime ();

	//std::cout << "Making Supervoxel structures" << std::endl;
	makeSupervoxels(supervoxel_clusters);//得到聚类结果-超体素map<int,supervoxel>
...
////////////////////////////////////////////////
template <typename PointT> void
pcl::clustering<PointT>::transformFunction(PointT &p)
{
	p.x /= p.z;
	p.y /= p.z;
	p.z = std::log(p.z);
...
template <typename PointT> void
pcl::clustering<PointT>::setUseSingleCameraTransform(bool val)
{
...
pcl::clustering<PointT>::prepareForSegmentation()
{
	...
	if ((use_default_transform_behaviour_ && input_->isOrganized())
		...
		adjacency_octree_->setTransformFunction(boost::bind(&clustering::transformFunction, this, _1));

	adjacency_octree_->addPointsFromInputCloud();
	computeVoxelData();//计算体素中心点云和对应法线
	return true;
}

template <typename PointT> void
pcl::clustering<PointT>::computeVoxelData()
{
	...
	if (input_normals_)
	{
		...
		for (leaf_itr = adjacency_octree_->begin(); leaf_itr != adjacency_octree_->end(); ++leaf_itr)
		{
			...
			voxel_data.owner_ = 0;
			voxel_data.distance_ = std::numeric_limits<float>::max();//编译器允许float类型数最大值
	...
	else //Otherwise just compute the normals
	{
		for (leaf_itr = adjacency_octree_->begin(); leaf_itr != adjacency_octree_->end(); ++leaf_itr)
		{
			...
			indices.reserve(81);//预留大小：26*3+3
			//Push this point
...
template <typename PointT> void
pcl::clustering<PointT>::expandSupervoxels(int depth)
{
...
template <typename PointT> void
pcl::clustering<PointT>::makeSupervoxels(std::map<uint32_t, typename Supervoxel<PointT>::Ptr > &supervoxel_clusters)
{
...
template <typename PointT> void
pcl::clustering<PointT>::selectInitialSupervoxelSeeds(std::vector<int> &seed_indices)
{
	//初始化种子八叉树
	...
	float search_radius = 0.5f*seed_resolution_;//Rsearch
...
template <typename PointT> void
pcl::clustering<PointT>::createSupervoxelHelpers(std::vector<int> &seed_indices)
{
...
template <typename PointT> void
pcl::clustering<PointT>::reseedSupervoxels()
{
	//Go through each supervoxel and remove all it's leaves
	for (typename HelperListT::iterator sv_itr = supervoxel_helpers_.begin(); sv_itr != supervoxel_helpers_.end(); ++sv_itr)
	{
		sv_itr->removeAllLeaves();//移除了叶子，还有根部，ower_和distance_清零
	}
	std::vector<int> closest_index;
	std::vector<float> distance;
	...
	for (typename HelperListT::iterator sv_itr = supervoxel_helpers_.begin(); sv_itr != supervoxel_helpers_.end(); ++sv_itr)
	{
		...
		voxel_kdtree_->nearestKSearch(point, 1, closest_index, distance);//random？
...
pcl::clustering<PointT>::voxelDataDistance(const VoxelData &v1, const VoxelData &v2) const
{
	float spatial_dist = (v1.xyz_ - v2.xyz_).norm() / seed_resolution_;
	float color_dist = (v1.rgb_ - v2.rgb_).norm() / 255.0f;
	float cos_angle_normal = 1.0f - std::abs(v1.normal_.dot(v2.normal_));//没有使用FPFH
	return  cos_angle_normal * normal_importance_ + color_dist * color_importance_ + spatial_dist * spatial_importance_;
}
////////////////////////////////////////////////////////
template <typename PointT> void
pcl::clustering<PointT>::getSupervoxelAdjacencyList(VoxelAdjacencyList &adjacency_list_arg) const
{
...
template <typename PointT> void
pcl::clustering<PointT>::getSupervoxelAdjacency(std::multimap<uint32_t, uint32_t> &label_adjacency) const
{
...
pcl::clustering<PointT>::getVoxelCentroidCloud() const
{
	typename PointCloudT::Ptr centroid_copy(new PointCloudT);
	copyPointCloud(*voxel_centroid_cloud_, *centroid_copy);//private：voxel_centroid_cloud_ 
	return centroid_copy;
}

template <typename PointT> pcl::PointCloud<pcl::PointXYZL>::Ptr
pcl::clustering<PointT>::getLabeledVoxelCloud() const
{
...
template <typename PointT> pcl::PointCloud<pcl::PointXYZL>::Ptr
pcl::clustering<PointT>::getLabeledCloud() const
{
...
pcl::clustering<PointT>::makeSupervoxelNormalCloud(std::map<uint32_t, typename Supervoxel<PointT>::Ptr > &supervoxel_clusters)
{
...
///////////////////////////////////////////////////////////
template <typename PointT> float
pcl::clustering<PointT>::getVoxelResolution() const
{
	return (resolution_);
}
template <typename PointT> void
pcl::clustering<PointT>::setVoxelResolution(float resolution)
{
...
template <typename PointT> float
pcl::clustering<PointT>::getSeedResolution() const
{
	return (seed_resolution_);
}
template <typename PointT> void
pcl::clustering<PointT>::setSeedResolution(float seed_resolution)
{
	seed_resolution_ = seed_resolution;
}

template <typename PointT> void
pcl::clustering<PointT>::setColorImportance(float val)
{
	color_importance_ = val;
}
template <typename PointT> void
pcl::clustering<PointT>::setSpatialImportance(float val)
{
	spatial_importance_ = val;
}
template <typename PointT> void
pcl::clustering<PointT>::setNormalImportance(float val)
{
	normal_importance_ = val;
}

template <typename PointT> int
pcl::clustering<PointT>::getMaxLabel() const
{
	int max_label = 0;
	for (typename HelperListT::const_iterator sv_itr = supervoxel_helpers_.cbegin(); sv_itr != supervoxel_helpers_.cend(); ++sv_itr)
	{
...
///////////////////////////////////////////////
template<> void
pcl::clustering<pcl::PointXYZRGB>::VoxelData::getPoint(pcl::PointXYZRGB &point_arg) const;
template<> void
pcl::clustering<pcl::PointXYZRGBA>::VoxelData::getPoint(pcl::PointXYZRGBA &point_arg) const;

template<typename PointT> void
pcl::clustering<PointT>::VoxelData::getPoint(PointT &point_arg) const
{
...
template <typename PointT> void
pcl::clustering<PointT>::VoxelData::getNormal(Normal &normal_arg) const
{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointT> void
pcl::clustering<PointT>::SupervoxelHelper::addLeaf(LeafContainerT* leaf_arg)
{
...
template <typename PointT> void
pcl::clustering<PointT>::SupervoxelHelper::removeLeaf(LeafContainerT* leaf_arg)
{
	leaves_.erase(leaf_arg);
}
template <typename PointT> void
pcl::clustering<PointT>::SupervoxelHelper::removeAllLeaves()
{
...
template <typename PointT> void
pcl::clustering<PointT>::SupervoxelHelper::expand()
{
...
template <typename PointT> void
pcl::clustering<PointT>::SupervoxelHelper::refineNormals()
{
	typename SupervoxelHelper::iterator leaf_itr;
...
template <typename PointT> void
pcl::clustering<PointT>::SupervoxelHelper::updateCentroid()
{
...
/////////////////////////////////////////////
template <typename PointT> void
pcl::clustering<PointT>::SupervoxelHelper::getVoxels(typename pcl::PointCloud<PointT>::Ptr &voxels) const
{
...
template <typename PointT> void
pcl::clustering<PointT>::SupervoxelHelper::getNormals(typename pcl::PointCloud<Normal>::Ptr &normals) const
{
...
template <typename PointT> void
pcl::clustering<PointT>::SupervoxelHelper::getNeighborLabels(std::set<uint32_t> &neighbor_labels) const
{
...
namespace pcl
{
	namespace octree
	{
		...
		template<>
		void
			pcl::octree::OctreePointCloudAdjacencyContainer<pcl::PointXYZRGB, pcl::clustering<pcl::PointXYZRGB>::VoxelData>::addPoint(const pcl::PointXYZRGB &new_point);

		template<>
		void
			pcl::octree::OctreePointCloudAdjacencyContainer<pcl::PointXYZRGBA, pcl::clustering<pcl::PointXYZRGBA>::VoxelData>::addPoint(const pcl::PointXYZRGBA &new_point);

		//Explicit overloads for RGB types
		template<> void
			pcl::octree::OctreePointCloudAdjacencyContainer<pcl::PointXYZRGB, pcl::clustering<pcl::PointXYZRGB>::VoxelData>::computeData();

		template<> void
			pcl::octree::OctreePointCloudAdjacencyContainer<pcl::PointXYZRGBA, pcl::clustering<pcl::PointXYZRGBA>::VoxelData>::computeData();

		//Explicit overloads for XYZ types
		template<>
		void
			pcl::octree::OctreePointCloudAdjacencyContainer<pcl::PointXYZ, pcl::clustering<pcl::PointXYZ>::VoxelData>::addPoint(const pcl::PointXYZ &new_point);

		template<> void
			pcl::octree::OctreePointCloudAdjacencyContainer<pcl::PointXYZ, pcl::clustering<pcl::PointXYZ>::VoxelData>::computeData();
	}
}
